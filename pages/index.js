...
export async function getServerSideProps() {
  const initialTasks = [
    {
      ...
      business_value: faker.helpers.arrayElement([
        0, 1, 2, 3, 5, 8, 13, 20, 40,
      ]),
      time_criticality: faker.helpers.arrayElement([
        0, 1, 2, 3, 5, 8, 13, 20, 40,
      ]),
      risk_opportunity: faker.helpers.arrayElement([
        0, 1, 2, 3, 5, 8, 13, 20, 40,
      ]),
  ...
  const tasksWithWSJF = await Promise.all(
    initialTasks.map(calculateInitiateTasks),
  );
...
export default function Home({ initialTasks }) {
  ...
  const [isLoadingFetchingChildren, setIsLoadingFetchingChildren] =
    useState(false);
  const [isLoadingGenerateSubtasks, setIsLoadingGenerateSubtasks] =
    useState(false);
  const [parentDetail, setParentDetail] = useState(null);
  const [abortFetchingChildrenController, setAbortFetchingChildrenController] =
    useState(null);
  const [isIssueKeyParentNotFound, setIsIssueKeyParentNotFound] =
    useState(false);
  ...
  const [isSecondarySubmitVisible, setIsSecondarySubmitVisible] =
    useState(false);
  const [isSecondSubmitDisabled, setIsSecondSubmitDisabled] = useState(false);
  ...
  const [isModalFetchChildrenVisible, setIsModalFetchChildrenVisible] =
    useState(false);
  ...
  useEffect(() => {
    if (initialTasks && initialTasks.length > 0) {
      // Ensure initialTasks has data
      const allRowsHaveWSJF = initialTasks.every(
        (taskItem) => taskItem.wsjf_score !== null,
      );
      console.log("All rows have WSJF:", allRowsHaveWSJF);
      setIsSecondarySubmitVisible(allRowsHaveWSJF);
    }
  }, [initialTasks]);
  ...
  const handleOk = async () => {
    ...
    setIsSecondarySubmitVisible(false); // Hide secondary submit when fetching children
      ...
      if (response?.status === 200) {
        ...
        if (result.length === 0) {
          Modal.warning({
            title: "Warning",
            content:
              "The selected issue does not have any associated sub-tasks.",
          });
        }
  ...
  const [task, setTask] = useState(initialTasks || []); // Ensure it's either initialTasks or an empty array
  ...
  useEffect(() => {
    const areAllFieldsFilled = task.every((taskItem) => {
      return (
        taskItem.issue_key &&
        taskItem.issue_summary &&
        taskItem.business_value &&
        taskItem.time_criticality &&
        taskItem.risk_opportunity &&
        taskItem.job_duration
      );
    });
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        if (!response.data.key) {
          ...
          if (
            result.issuetype.id !== "10000" &&
            result.issuetype.id !== "10001"
          ) {
  ...
  const handleIssueKeyChange = (newValue, id) => {
    // Just set the value for issue_key without validation
    setTask((prevTask) =>
      prevTask.map((task) =>
        task.id === id ? { ...task, issue_key: newValue.toUpperCase() } : task,
      ),
    );
  };

  const handleIssueKeyBlur = async (issueKey, id) => {
    const regex = /^[A-Za-z]{1,10}-\d{1,5}$/;

    // Validate the input after blur
    if (regex.test(issueKey)) {
      try {
        const response = await jiraClient.post(issueKey);
        if (response?.status === 200) {
          const issueSummary = response.data.fields.summary;
          // Update the Issue Summary field
          setTask((prevTask) =>
            prevTask.map((task) =>
              task.id === id ? { ...task, issue_summary: issueSummary } : task,
            ),
          );
        }
      } catch (error) {
  ...
  const columns = [
    {
      title: <Tooltip title="Jira issue key">Issue Key</Tooltip>,
      dataIndex: "issue_key",
      key: "issue_key",
      align: "center",
      width: 150,
      render: (text, record) => (
        <Input
          value={text} // Ensuring it displays the current value correctly
          type="text" // Explicitly ensure this is a text input
          onChange={(e) => handleIssueKeyChange(e.target.value, record.id)} // Update the issue key
          onBlur={() => handleIssueKeyBlur(record.issue_key, record.id)} // Validate and fetch on blur
          placeholder="Enter Issue Key (e.g., ABC-123)"
        />
      ),
    },
    {
      title: <Tooltip title="Jira issue Summary">Issue Summary</Tooltip>,
      ...
      render: (text, record) => (
        <Input
          value={text}
          onChange={(e) =>
            handleValueChange(e.target.value, record.id, "issue_summary")
          }
        />
      ),
    },
      ...
      render: (text, record) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <Input
            value={text}
            onChange={(e) =>
              handleValueChange(e.target.value, record.id, "business_value")
            }
      ...
      render: (text, record) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <Input
            value={text}
            onChange={(e) =>
              handleValueChange(e.target.value, record.id, "time_criticality")
            }
            style={{ width: "60px", textAlign: "center" }}
          />
          <Button
      ...
      render: (text, record) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <Input
            value={text}
            onChange={(e) =>
              handleValueChange(e.target.value, record.id, "risk_opportunity")
            }
            style={{ width: "60px", textAlign: "center" }}
          />
          <Button
      ...
      render: (text, record) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <Input
            value={text}
            onChange={(e) =>
              handleValueChange(e.target.value, record.id, "job_duration")
            }
            style={{ width: "60px", textAlign: "center" }}
          />
          <Button
    ...
    >
      <div style={{ marginTop: "40px", flex: "1 0 auto", textAlign: "center" }}>
        ...
        >
          ...
          >
            {/* Main Add Task or Submit Button */}
            {isAddTaskEnabled ? (
              <Button
                className="main-button"
                icon={<PlusOutlined />}
                onClick={() => {
                  if (task.length >= 10) {
                    setIsPaywallVisible(true); // Show paywall if more than 10 tasks
                  } else {
                    addTask(); // Allow adding a task
                  }
                }}
              >
                Add Task
              ...
              <Button
                ...
                icon={<FireOutlined />}
                onClick={async () => {
                  setIsLoadingSubmitJira(true); // Show loading while submitting
                  task.forEach((taskItem) => {
                    if (taskItem.issue_key) {
                      updateTask(taskItem.issue_key, taskItem.wsjf_score); // Update task WSJF score
                    }
                  });
                  setIsLoadingSubmitJira(false); // Stop loading
                }}
                disabled={isSubmitButtonDisabled} // Disable submit if conditions aren't met
              >
                Submit
              </Button>
            )}
            ...
            {isSecondarySubmitVisible && (
              <Button
                type="primary"
                className="main-button"
                icon={<FireOutlined />} // Same styling as the first submit button
                style={{ marginLeft: "10px" }} // Adds some margin between buttons
                onClick={() => {
      ...
      >
        ...
        >
          Enter the issue key of the Jira Epic or Story to fetch its children
          tasks.
        </Text>
        <Form form={form}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please input your Parent Issue Key!",
              },
              {
                pattern: /^[A-Za-z]{1,10}-\d{1,5}$/,
                message:
                  "Parent Issue Key should start with 1-10 alphabets followed by a dash and then 1-5 numbers. Example: KHO-12345",
              },
            ]}
          >
            <Input
              ...
              suffix={
                parentDetail?.summary &&
                !isIssueKeyParentNotFound &&
                !isCheckingParentIsError ? (
        ...
        {!isRemoveErrorMessage && (
          ...
          >
            {isCheckingParentIsError && (
              <Text type="danger">Issue type should be a Epic or Story</Text>
            )}
            {isIssueKeyParentNotFound && (
              <Text type="danger">Given Issue Key not found</Text>
            )}
            {parentDetail?.summary &&
              (parentDetail.issuetype.id === "10001" ? (
            ...
            {parentDetail && (
              ...
              >
                {parentDetail?.key}: {parentDetail?.summary}
              </Text>
            )}
          </div>
        )}
      ...
      >
        <p className="modal-subtitle">
          Get Unlimited digital access free for one month Unlimited Digital
          Access
        </p>
        <Button
          type="primary"
      ...
      <Modal
        ...
        footer={null}
      >
        <p className="modal-payment-subtitle">
          Make Your payment securely through your bank account
        </p>
        <div
          style={{
            display: "flex",
        ...
        >
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please input your cardholder name!" },
              {
                ...
                message:
                  "Name should only contain alphabets and spaces, max 40 characters",
              },
            ]}
          >
            <Input
          ...
          <Form.Item>
            <Button
              type="primary"
              onClick={() => form.submit()}
              disabled={isSubmitParentEnabled}
            >
      ...
      >
        <iframe
          src="https://giphy.com/embed/j4wrsOnwaAhjgMsC1g"
          width="480"
          height="270"
          frameBorder="0"
